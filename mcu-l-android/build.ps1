...
if ($QuoteApiEndpoint) {
    ...
    Set-Content -LiteralPath (Join-Path $AssetRoot 'quote-config.js') -Value "window.MCUS_QUOTES_ENABLED=$QuoteEnabledJson;window.MCUS_QUOTE_API=$QuoteApiJson;" -Encoding UTF8
}
...
& $Aapt2 link -o $UnsignedApk --manifest (Join-Path $ProjectRoot 'AndroidManifest.xml') -I $AndroidJar -A $AssetRoot --min-sdk-version 24 --target-sdk-version 35 --version-code 26 --version-name '1.4x' $CompiledResources
...
$FinalApk = Join-Path $DistRoot 'MCUS-1.4x-debug.apk'
