(() => {
  ...
  function updateAnalysisScope() {
    ...
    const fullOutputText = $("#fullOutputFormat")?.value === "hdf5_mp4" ? "HDF5 + MP4 兼容格式" : "LeRobot 双手 21 点 + Body 格式", fullExportWarning = fullPipelineExportWarning();
  ...
  function openAnalysisScope(operation, trigger = document.activeElement) {
    ...
    $("#analysisScopeTitle").textContent = full ? "Full 标准数据集范围" : curation ? "数据质量清洗范围" : smoothing ? "视频平滑范围" : projection ? "手部二维投影归正范围" : behavior ? "VLM 行为标注范围" : trimming ? "YOLOE 无动作剪切范围" : qwenTrimming ? "Qwen 片段剪切范围" : "SLAM 位姿恢复范围";
    $("#analysisScopeKind").textContent = full ? "平滑 → S1-S5/C3（Action 可选）→ VLM → C1/C2 → LeRobot / 兼容格式导出" : curation ? "S1-S5 → C3 → 非红片段 VLM → C1/C2" : smoothing ? "光流稳像与轻度运动模糊缓解" : projection ? "MediaPipe / AlicePose 全帧双手观测 → 左右手唯一关联 → 骨长与刚性掌部约束 → 视频帧对齐 3D 快照" : behavior ? "Qwen-VLM 高层任务 + 可变长度细阶段 + Joint 边界校正" : trimming ? "YOLOE 物体与手/夹爪距离分析" : qwenTrimming ? "Qwen-VLM 有效操作 / 无效片段判定" : "SLAM / Visual Odometry 初始位姿恢复";
